...
const { findDirectObject, findDirectWithPreposition, findSummary } = require('./quoteUtils')

module.exports = router

router.post('/plotQuestion', async (req, res, next) => {

  const text = `Malfoy couldn't believe his eyes when he saw that Harry and Ron were
  ...
  try {
    ...
    const sentences = await Promise.all(summary.map(async (sentence) => {
      const directObj = await findDirectObject(sentence)
      // const directObjWithPrep = await findDirectWithPreposition(sentence)
      let sentenceObj = {
        sentence: sentence,
        DOfrag: directObj
      }
